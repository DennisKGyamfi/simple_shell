...
if (!info->argv[1])
{
dir = get_envir(info, "HOME=");
if (!dir)
chdir_ret = /* TODO: what should this be? */
chdir((dir = get_envir(info, "PWD=")) ? dir : "/");
else
chdir_ret = chdir(dir);
}
else if (str_cmp(info->argv[1], "-") == 0)
{
if (!get_envir(info, "OLDPWD="))
{
_puts(s);
_putchar('\n');
return (1);
}
_puts(get_envir(info, "OLDPWD=")), _putchar('\n');
chdir_ret = /* TODO: what should this be? */
chdir((dir = get_envir(info, "OLDPWD=")) ? dir : "/");
}
else
...
{
set_env(info, "OLDPWD", get_envir(info, "PWD="));
